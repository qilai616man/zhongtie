...
import com.brainsoon.outlookactivity.OutLookActivity;
import com.brainsoon.outlookactivity.OutLookData;
import com.brainsoon.outlookactivity.OutlineItem;
import com.ereader.zip.openzip;
...
public class MuPDFActivity extends Activity implements
	...
	private final int OUTLINE_REQUEST = 1;
	private final int PRINT_REQUEST = 2;
	private final int FILEPICK_REQUEST = 3;
	private MuPDFCore core;
	private String mFileName;
	private MuPDFReaderView mDocView;
	private View mButtonsView;
	private View annotateMenuView;
	private View cancelView;
	private Button annotateCancelImageButton;
	private Button mAnnotateOkImageButton;
	private ImageButton mMarkImageButton;
	private TextView mAnnotateTipsTextView;
	private EditText mAnnotateEditTextView;
	private TextView mAnnotateTextView;
	private PopupWindow annotateMenu;
...
//	 private ImageButton mReflowButton;
	...
	private ImageButton mGoPageButton;
	private ImageButton mCopyButton;
	//	private ImageButton mCopyButton1;
...
//	 private ImageButton mLinkButton;
	...
	private PDFMarkDbAdapter mPDFMarkDbAdapter;
	private boolean mIsHaveMark = false;
	private boolean mIsPageMoved = false;
	...
	private MuPDFCore openFile(String path) {
		...
		try {
			...
			try {
				InputStream in = getResources().getAssets().open("error.pdf"); // 从Resources中raw中的文件获取输入流
	...
	private MuPDFCore openBuffer(byte buffer[], String fileName) {
...
//				String fileString = uri.getPath();
				String fileString = intent.getStringExtra("unzipPDFFullName");
				if (fileString.startsWith("file://")) {
					fileString = uri.toString().substring(7);
				}

				File pdfFile = new File(
						intent.getStringExtra("unzipPDFFullName"));
				if (!pdfFile.isFile()) {
					if (fileString.endsWith(".tdp")) {
						openzip zip = new openzip(fileString);
						zip.unzipPDF(pdfFile.toString());
					}
				}
				core = openFile(pdfFile.toString());
			}
			SearchTaskResult.set(null);
			if (core != null && core.needsPassword()) {
				String aesKey = intent.getStringExtra("aesKey");
				core.authenticatePassword(aesKey);
		...
		mPDFMarkDbAdapter = new PDFMarkDbAdapter(this);
	...
	public void createUI(Bundle savedInstanceState) {
		...
		mDocView = new MuPDFReaderView(this) {
			...
			protected void onMoveToChild(int i) {
				...
				mPDFMarkDbAdapter.open(mFileName);
				if (mPDFMarkDbAdapter.exist(Integer.toString(i))) {
					mIsHaveMark = true;
					// mBookMarkButton.setImageResource(R.drawable.ic_bookmark_selected);
					mBookMarkButton
							.setImageResource(R.drawable.icon_fb_annotation);
					mMarkImageButton.setVisibility(View.VISIBLE);
				} else {
					mIsHaveMark = false;
					mBookMarkButton
							.setImageResource(R.drawable.icon_fb_annotation);
					mMarkImageButton.setVisibility(View.INVISIBLE);
				}
				mPDFMarkDbAdapter.close();
				mAnnotateTipsTextView.setVisibility(View.INVISIBLE);
		...
		annotateMenuView = inflater.inflate(R.layout.annotate_menu, null);
		annotateMenu = new PopupWindow(annotateMenuView,
		...
		mPageSlider
				.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
					public void onStopTrackingTouch(SeekBar seekBar) {
						mOldPage = mDocView.getDisplayedViewIndex();
						mDocView.setDisplayedViewIndex((seekBar.getProgress() + mPageSliderRes / 2)
								/ mPageSliderRes);
						mGoPageButton
								.setBackgroundResource(R.drawable.ic_progress_back);
						mIsPageMoved = true;
					}
		...
		mBookMarkButton.setOnClickListener(new View.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				if (annotateMenu.isShowing()) {
					annotateMenu.dismiss();
				} else if (mIsHaveMark) {
					mAnnotateTextView.setText("修改批注");
					mPDFMarkDbAdapter.open(mFileName);
					String mark = mPDFMarkDbAdapter.get(Integer
							.toString(mDocView.getDisplayedViewIndex()));
					mPDFMarkDbAdapter.close();
					if (mark != null) {
						mAnnotateEditTextView.setText(mark);
					} else {
						mAnnotateEditTextView.setText("");
					}
					MuPDFActivity.this.annotateMenu.showAtLocation(
							mBookMarkButton, Gravity.BOTTOM, 0, 0);
					MuPDFActivity.this.annotateMenu
							.showAsDropDown(annotateMenuView);
				} else if (!mIsHaveMark) {
					mAnnotateTextView.setText("添加批注");
					mAnnotateEditTextView.setText("");
					MuPDFActivity.this.annotateMenu.showAtLocation(
							mBookMarkButton, Gravity.BOTTOM, 0, 0);
					MuPDFActivity.this.annotateMenu
		...
		mBookMarkButton.setOnLongClickListener(new OnLongClickListener() {

			@Override
			public boolean onLongClick(View v) {
				if (mIsHaveMark) {
					AlertDialog alert = new AlertDialog.Builder(
							MuPDFActivity.this).setTitle("确认")
							.setMessage("确定要删除该批注吗？")
							.setPositiveButton("是", new OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog,
													int which) {
									mPDFMarkDbAdapter.open(mFileName);
									mPDFMarkDbAdapter.delete(Integer
											.toString(mDocView
...
//				TextWord[][] textLine = core.textLines(index);
//				StringBuffer buffer = new StringBuffer();
//				for(int i = 0 ;i < textLine.length ;i ++){
//					for(int j = 0 ;j < textLine[i].length ;j++){
//						buffer.append(textLine[i][j].w);
//					}
//					buffer.append("\n");
//				}
//				String content = buffer.toString();
//				content = content.replaceAll("摇", " ");
//				content = content.replaceAll("郾", ".");
//				Ln.d("MuPDFActivity:createUI:content:" + content);
//				ClipboardManager cmb = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
//				cmb.setText(content.trim());
//				Toast.makeText(MuPDFActivity.this ,"已复制到粘贴板" ,Toast.LENGTH_SHORT).show();
//			}
//		});
...
//		 });
		...
		mAnnotateOkImageButton.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				annotateMenu.dismiss();
				String mark = mAnnotateEditTextView.getText().toString();
				if (mark.compareTo("") != 0) {
					int index = mDocView.getDisplayedViewIndex();
					mPDFMarkDbAdapter.open(mFileName);
					if (mIsHaveMark) {
						mPDFMarkDbAdapter.update(mark, Integer.toString(index));
					} else {
						mPDFMarkDbAdapter.add(mark, Integer.toString(index));
					}
					mPDFMarkDbAdapter.close();
					mBookMarkButton
							.setImageResource(R.drawable.icon_fb_annotation);
		...
		mMarkImageButton.setOnClickListener(new View.OnClickListener() {

			@Override
			public void onClick(View v) {
				if (mAnnotateTipsTextView.isShown()) {
					mAnnotateTipsTextView.setVisibility(View.INVISIBLE);
				} else {
					mPDFMarkDbAdapter.open(mFileName);
					String mark = mPDFMarkDbAdapter.get(Integer
							.toString(mDocView.getDisplayedViewIndex()));
					mPDFMarkDbAdapter.close();
					mAnnotateTipsTextView.setText(mark);
					mAnnotateTipsTextView.setVisibility(View.VISIBLE);
				}

			}
		});
		mGoPageButton
				.setBackgroundResource(R.drawable.ic_progress_back_disable);
		mGoPageButton.setOnClickListener(new View.OnClickListener() {

			private boolean mStateButton = false;
			...
			public void onClick(View v) {
				...
				mDocView.setDisplayedViewIndex(mOldPage);
				mOldPage = page;
				if (mStateButton) {
					mGoPageButton
							.setBackgroundResource(R.drawable.ic_progress_back);
					...
					mGoPageButton
							.setBackgroundResource(R.drawable.ic_progress_forward);
				}
				mStateButton = !mStateButton;
			}
		});
		...
		if (core.hasOutline()) {
			mOutlineButton.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					...
					OutLookData data = new OutLookData(outline,
							OutLookData.TYPE_PDF, mFileName);
					if (outline != null) {
						Intent intent = new Intent(MuPDFActivity.this,
								OutLookActivity.class);
						intent.putExtra(OutLookData.OUT_LINE, data);
...
//		 : Color.argb(0xFF, 255, 255, 255));
	...
	private void showButtons() {
		...
		if (!mButtonsVisible) {
			...
			anim = new TranslateAnimation(0, 0,
					mLinearLayoutSeekBar.getHeight(), 0);
			anim.setDuration(200);
			anim.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation animation) {
			...
			mLinearLayoutSeekBar.startAnimation(anim);
		}
	}
	...
	private void hideButtons() {
		if (mButtonsVisible) {
			...
			anim = new TranslateAnimation(0, 0, 0,
					mLinearLayoutSeekBar.getHeight());
			anim.setDuration(200);
			anim.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation animation) {
				...
				public void onAnimationEnd(Animation animation) {
					mLinearLayoutSeekBar.setVisibility(View.INVISIBLE);
				}
			});
			mLinearLayoutSeekBar.startAnimation(anim);
		}
	...
	private void makeButtonsView() {
		...
		mGoBackButton = (ImageButton) mButtonsView
...
//		 (ImageButton)mButtonsView.findViewById(R.id.reflowButton);
		...
		mAnnotateTipsTextView = (TextView) mButtonsView
				.findViewById(R.id.textViewTips);
		mMarkImageButton = (ImageButton) mButtonsView
				.findViewById(R.id.imageButtonMark);
		cancelView = annotateMenuView.findViewById(R.id.cancelView);
		annotateCancelImageButton = (Button) annotateMenuView
				.findViewById(R.id.imageButtonAnnotateCancel);
		mAnnotateOkImageButton = (Button) annotateMenuView
				.findViewById(R.id.imageButtonAnnotateOk);
		mAnnotateEditTextView = (EditText) annotateMenuView
				.findViewById(R.id.editTextAnnotate);
		mAnnotateTextView = (TextView) annotateMenuView
				.findViewById(R.id.textViewAnnotate);
		mLinearLayoutSeekBar = (LinearLayout) mButtonsView
				.findViewById(R.id.linearLayoutSeekBar);
		mGoPageButton = (ImageButton) mButtonsView
				.findViewById(R.id.imageButtonPageGoback);
//		mCopyButton1 =(ImageButton) mButtonsView.findViewById(R.id.copyButton1);
//		 mLinkButton =
//		 (ImageButton)mButtonsView.findViewById(R.id.linkButton);
		// mMoreButton =
		// (ImageButton)mButtonsView.findViewById(R.id.moreButton);
		mTopBarSwitcher.setVisibility(View.INVISIBLE);
		// mPageNumberView.setVisibility(View.INVISIBLE);
		mInfoView.setVisibility(View.INVISIBLE);
	...
	public void OnAcceptButtonClick(View v) {
		...
		case CopyText:
			...
			mTopBarMode = TopBarMode.More;
			Ln.d("MuPDFActivity:createUI:mTopBarMode:" + mTopBarMode);
			...
			if(content.length() > 500){
				showInfo(success ? getString(R.string.copied_to_clipboard1)
						: getString(R.string.no_text_selected));
//				Toast.makeText(MuPDFActivity.this ,"您复制的文本内容大于150个字，将只会粘贴前150个字" ,Toast.LENGTH_SHORT).show();
