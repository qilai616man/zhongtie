...
public class BookCollectionShadow extends AbstractBookCollection implements ServiceConnection {
	...
	public synchronized void bindToService(Context context, Runnable onBindAction) {
		...
		if (myInterface != null && myContext == context) {
			...
			boolean is=context.bindService(
				new Intent(context, LibraryService.class),
				this,
				LibraryService.BIND_AUTO_CREATE
			);
			Log.i("ccccccccc","bindService==="+is);
			myContext = context;
		}
	}
	SearchLocalCountInterface searchLocalCountInterface;
	public LibraryInterface MyBindToService(Context context, SearchLocalCountInterface searchLocalCountInterface){
		this.myContext=context;
		issynchronized=false;
		this.searchLocalCountInterface=searchLocalCountInterface;
		context.bindService(
				new Intent(context, LibraryService.class),
				this,
				LibraryService.BIND_AUTO_CREATE
	...
	public synchronized Book getBookByFile(ZLFile file) {
		...
		Log.i("cccccc","myInterface=="+myInterface);
		if (myInterface == null) {
			return null;
		}
		try {
	...
	public synchronized void onServiceConnected(ComponentName name, IBinder service) {
		...
		Log.i("cccccc","success=="+myInterface);
		...
		if(!issynchronized){
			searchLocalCountInterface.resfreshCount();
		}
