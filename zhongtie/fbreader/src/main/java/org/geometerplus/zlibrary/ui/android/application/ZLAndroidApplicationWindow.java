...
public final class ZLAndroidApplicationWindow extends ZLApplicationWindow {
	...
	//2013
	public void addMenuItem(Menu menu, String actionId, Integer iconId, String name, boolean showInActionBar,boolean always) {
		if (name == null) {
		...
		if (iconId != null) {
			...
			//final FBReader activity = ((ZLAndroidLibrary)ZLAndroidLibrary.Instance()).getActivity();
			if(always){
				menuItem.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
			}else if (showInActionBar) {
	...
	@Override
	public void refresh() {
		//Log.d("ZLAndroidApplicationWindow刷新", "begin");
	...
	protected void processException(Exception exception) {
		if(exception instanceof Exception){
			Log.d("自定义异常", exception.getMessage());
		}
