...
import org.geometerplus.zlibrary.ui.android.options.ZLBooleanOption;
import org.geometerplus.zlibrary.ui.android.options.ZLColorOption;
import org.geometerplus.zlibrary.ui.android.options.ZLEnumOption;
import org.geometerplus.zlibrary.ui.android.options.ZLIntegerRangeOption;
import org.geometerplus.zlibrary.ui.android.options.ZLStringOption;
	...
	public void openBook(final Book book, final Bookmark bookmark, final Runnable postAction) {
		if (book != null || Model == null) {
			runWithMessage("loadingBook", new Runnable() {
				public void run() {
					//强制重新加载
					openBookInternal(book, bookmark, true);
//					getSearchCount(book);
					if (book != null) {
...
//synchronized
    void openBookInternal(Book book, Bookmark bookmark, boolean force) {
		if (book == null) {
	...
	synchronized void getSearchCount(Book book) {
		...
		try {
			Model = BookModel.createModel(book);
			Log.i("Model=",Model.getTextModel().search("的",0,100,true)+"");
		} catch (BookReadingException e) {
			processException(e);
		}
	}
