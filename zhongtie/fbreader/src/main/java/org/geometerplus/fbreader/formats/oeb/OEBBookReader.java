...
class OEBBookReader extends ZLXMLReaderAdapter implements XMLNamespaces {
	...
	void readBook(ZLFile file,String aesKey) throws BookReadingException {
		myFilePrefix = MiscUtil.htmlDirectoryPrefix(file);

		Log.i("cccccc","oebbookReader");
		myIdToHref.clear();
		...
		for (String name : myHtmlFileNames) {
			...
			//2013年12月17日改为内容文件都需要解密
			xhtmlFile.setNeedDecryption(true);
			xhtmlFile.setAesKey(aesKey);
	...
	public boolean startElementHandler(String tag, ZLStringMap xmlattributes) {
		...
		if (MANIFEST == tag) {
			...
			//读取内容文件逻辑，opf中 manifest 节点中存在，并且spine引用
