...
public abstract class BookModel {
	public static BookModel createModel(Book book) throws BookReadingException {
		...
		Log.d("生成模型", "using plugin: " + plugin.supportedFileType() + "/" + plugin.type());
		...
		switch (plugin.type()) {
			case NATIVE:
				...
				Log.i("cccccccccc","plugintype=="+model);
				plugin.readModel(model,book.getAesKey(),"/stylesheet3.css");
				break;
			case JAVA:
				model = new JavaBookModel(book);
				plugin.readModel(model,book.getAesKey(), book.getStylesheetPath());
				break;
			default:
