...
public class ParallaxRecyclerAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<PgBookForLibraryListEntity> mData;
    ...
    public void removeParallaxHeader(View header) {
        if (mHeader != null)
            mHeader.removeView(header);
    }
    ...
    public List<PgBookForLibraryListEntity> getData() {
        return mData;
    }

    public void setData(List<PgBookForLibraryListEntity> data) {
        mData = data;
        notifyDataSetChanged();
    }

    public void addData(List<PgBookForLibraryListEntity> data) {
        if (mData == null)
            mData = new ArrayList<>();
        mData.addAll(data);
        notifyDataSetChanged();
    ...
    public void addItem(PgBookForLibraryListEntity item, int position) {
        mData.add(position, item);
        notifyItemInserted(position + (mHeader == null ? 0 : 1));
    }

    public void setItem(PgBookForLibraryListEntity item, int position) {
        mData.set(position, item);
        notifyItemChanged(position + (mHeader == null ? 0 : 1));
    }
